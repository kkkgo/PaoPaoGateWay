...
- 15 `net_rec`选项：网络流量记录功能，可以记录网关连接了哪些域名、上传下载消耗了多少流量、客户端IP，并默认按照消耗流量的大小排序，该功能可以根据实际情况方便地调整分析自己的域名规则列表。设置为`yes`开启该功能后可以在web界面点击[记录]，选择下载表格或者在线加载。注意事项：
    - 1、重启或者修改ppgw的密码、Web端口、关了再开`net_rec`选项会导致数据清空。
    - 2、如果流量太小连接持续时间过短，有可能在记录之前连接已经关闭，流量会显示为0B。
    - 3、客户端IP记录的是最后一次连接该域名的客户端IP。
    - 4、数据下载没有身份验证。
    - 5、理论上会略微增加资源占用，取决于你的并发连接数量，可适当增加运行资源。
